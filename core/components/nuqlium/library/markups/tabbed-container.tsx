...
function TabContainer(props:any){
    ...
    return (
        <div className={`${content.page_width} ${content.panel_bg_color} ${content.margin_top_bottom_mobile} ${content.margin_top_bottom_desktop} px-1 md:px-2 min-[1537px]:px-0`}  data-nqe-widget="grid-container">
            <div className={`relative ${content.padding_top_bottom_mobile} ${content.padding_top_bottom_desktop}`}>
                <div className="relative">
                    {content.panel_title && content.panel_title != "" ?
                        <div className={`py-2 px-2 ${content.text_color} ${content.title_align}`}>
                            <p className={`${content.heading_font_size} ${content.title_font_weight} ${content.title_style} brand-font`}>{content.panel_title}</p>
                            <p className={`pt-3 text-base ${content.text_color}`}> {content.description} </p>
                        </div>	
                    :null}
                    <div className="">
...
function BuildButtons (props:any){
    ...
    return (
        ...
        {content.grid_feed_items.map((item:any) => {
                counter++;
                return (
                    item.zone_or_feed && item.zone_or_feed == "feed" ?
                        <div key={item.feed} onClick={(e) => props.handleActive(e.target)} className={`h-full py-2 group ${props.activeState == item.feed ? "activeGrid" : "nonActiveGrid" }`} data-nqe-grid="button" data-grid-feed={`${item.feed}`}>
                        ...
                        :
                        <div key={item.recommendation} onClick={(e) => props.handleActive(e.target)} className={`h-full py-2 group ${props.activeState == item.recommendation ? "activeGrid" : "nonActiveGrid" }`} data-nqe-grid="button" data-grid-feed={`${item.recommendation}`}>
                            <div className="flex justify-center"> 
                                <button className={`text-sm rounded-[4px] w-[128px] px-[30px] py-[16px] text-black font-medium border border-neutral-200 bg-neutral-200 group-[.activeGrid]:border group-[.activeGrid]:text-white group-[.activeGrid]:border-black group-[.activeGrid]:bg-black md:w-[200px] md:px-[40px] md:py-[16px]`} title={`${item.button_name}`}> {item.button_name} </button>
                            </div>
                        </div>
    ...
    return (
        <div>
            {content.grid_feed_items.map((item:any) => {
                ...
                return (
                        <div key={counter} className={`h-full group ${props.activeState == feedType ? "activeGrid" : "nonActiveGrid" }`}  data-grid-feed-container={`${feedType}`}>
