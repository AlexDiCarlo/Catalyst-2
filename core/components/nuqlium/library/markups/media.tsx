...
function Media(props:any){
    ...
    //console.log(content)
    return (
        <div className={`${content.page_width} h-full ${content.margin_top_bottom_mobile} ${content.margin_top_bottom_desktop}`} data-nqe-widget="nq-w-8">
            <div className={`relative h-full flex flex-col items-center  ${content.padding_top_bottom_mobile} ${content.padding_top_bottom_desktop}`}>
                    ...
                    <div className={`absolute h-full w-full ${content.content_image_overlay_color} ${content.content_image_overlay_opacity}`} data-nqe-url={`${content.image_url}`}></div>
                        <picture>
                            {content.bg_image_mobile && content.bg_image_mobile != "" ?
                                <source srcSet={`${content.bg_image_mobile}`} media="(max-width: 767px)"/>
                            :null
                            }
                            <img className={`w-full h-full object-cover group-[.nql-content-container]:rounded`} src={`${content.bg_image}`} alt={`${content.block_label}`} data-original-image={`{content.image_desktop}`} data-nqe-lazy-load="true" data-nqe-image-loaded-type="half" width="400" height="400" />
                        </picture>
                    </div>
                )
        break
